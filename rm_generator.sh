...
function generate_musics()
{
    ...
    NUM_MUSICS=`find . -type f -iname "*.mp3" ! -path "${MUSICS_OUT_FOLDER}/*" | wc -l` 
...
{
    cat << EOF
USAGE: ./rm_generator.sh [--log=[FULL | F | LOGONLY | L | READONLY | R | NONE | N ]
                          --mode=[SCRIPT | S | INTERACTIVE | I ]
                          --help
                          --version
                         ]
...
General options:
    ...
    --log=<LOG OPTION>              FULL: try to read logs from ./LOGS/, and does not use musics that already have hashes in there; 
    ...
    --mode=<MODE OPTION>            SCRIPT: try to execute in script mode, without prompting options for the user.            
...
function version()
{
    echo "Random Music Generator Script -- Version $SCRIPT_VERSION"          
}
...
declare -a MODE_OPTIONS=("SCRIPT" "INTERACTIVE" "S" "I") #TODO
MODE_OPT="" 
...
if [[ -n "$MODE_OPT" ]]; then 
    test_mode=`test_match_flags "$MODE_OPT" "${MODE_OPTIONS[@]}"`
    if [ "$test_mode" -eq "0" ]; then 
        echo "error: invalid argument --mode="$MODE_OPT""
        exit 11
    fi
fi
